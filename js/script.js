...
/**
 * Categorias que possuem "Páginas Especiais" (seções cadastradas em
 * editor.html) devem abrir pagina-categoria.html em vez da listagem
 * padrão de produtos (categorias.html). Aqui buscamos data/secoes.json
 * para descobrir quais categorias têm esse tipo de conteúdo.
 */
async function obterCategoriasComPaginaEspecial() {
    try {
        const resposta = await fetch("data/secoes.json");
        ...
        return new Set(
            (Array.isArray(secoes) ? secoes : [])
                .filter(secao => secao && secao.categoria)
                .map(secao => secao.categoria)
        );

    } catch (erro) {
        console.error("Erro ao carregar seções especiais do menu:", erro);
        return new Set();
    }
}

function linkParaCategoria(categoria, categoriasComPaginaEspecial) {
    return categoriasComPaginaEspecial.has(categoria.id)
        ? `pagina-categoria.html?categoria=${categoria.id}`
...
async function carregarMenuCategorias() {
    try {
        ...
        const itens = categorias.map(categoria => `
            <li><a href="${linkParaCategoria(categoria, categoriasComPaginaEspecial)}">${categoria.nome}</a></li>
        `).join("");

...
function montarMensagemWhatsapp(produto, variacao = null) {
    ...
    if (variacao) {
        ...
        texto = texto.replaceAll("{opcao}", variacao.nome);

    } else {

        // Não há variação selecionada: remove referências à opção
        // para a mensagem não ficar com {opcao} ou aspas vazias.
        texto = texto
            .replace(/\s*(?:da\s+)?opção\s*"\{opcao\}"\s*/i, " ")
            .replaceAll("{opcao}", "")
            .replace(/[ \t]{2,}/g, " ")
            .trim();
        ...
        >
            ...
            <div class="botao-lista-wrapper">
                ...
                >
                    ...
                    <span class="botao-lista-texto">${estaNaLista ? "Remover" : "Adicionar à Lista"}</span>
                </button>
            </div>
...
async function buscarProdutosCatalogo(termo) {
    ...
    const conteudoPadrao = paginaInicial
        ? [...document.querySelectorAll(".pagina-categorias"), document.getElementById("catalogo-secoes")]
        : [...document.querySelectorAll(".pagina-categorias")];
...
let variacaoModalAtual = null;

// js/produtos.js

function atualizarBotaoListaModal() {
    const btn = document.getElementById("botaoListaVariacao");
    const texto = document.getElementById("textoListaVariacao");
    const icone = btn?.querySelector("i");

    if (!btn || !produtoModalAtual) return;

    const variacao = variacaoModalAtual?.nome || null;
    const estaNaLista = ListaPedidos.existeVariacao(
        produtoModalAtual.id,
        variacao
    );

    btn.classList.toggle("ativo", estaNaLista);

    if (texto) {
        texto.textContent = estaNaLista ? "Remover da Lista" : "Adicionar à Lista";
    }

    if (icone) {
        icone.classList.toggle("fa-regular", !estaNaLista);
        icone.classList.toggle("fa-solid", estaNaLista);
    }
}
...
async function abrirModalVariacoes(produtoId) {
    ...
    function selecionarVariacao(variacao, label) {
        ...
        const mensagem = montarMensagemWhatsapp(produto, variacao);

        const numeroWhatsApp = CONFIG.empresa.whatsapp;
        botao.href = `https://wa.me/${numeroWhatsApp}?text=${mensagem}`;

        // 🔥 Atualiza o botão "Adicionar/Remover" para refletir
        // se ESSA variação específica já está na lista ou não
        atualizarBotaoListaModal();
...
document.addEventListener("DOMContentLoaded", () => {
    ...
    // 🔥 BOTÃO "ADICIONAR À LISTA" DENTRO DO MODAL
    // Usa uma variável global para evitar duplicação
    if (typeof window._modalListenerAdicionado === 'undefined') {
        window._modalListenerAdicionado = false;
    }

    if (!window._modalListenerAdicionado) {
        window._modalListenerAdicionado = true;

        const botaoListaModal = document.getElementById("botaoListaVariacao");

        if (botaoListaModal) {
            // Remove listener antigo para evitar duplicação
            const novoBotao = botaoListaModal.cloneNode(true);
            botaoListaModal.parentNode.replaceChild(novoBotao, botaoListaModal);

            // js/produtos.js - dentro do evento do botãoListaModal
            ...
            novoBotao.addEventListener("click", function (e) {
                ...
                const nomeCompleto = variacaoModalAtual
                    ? `${produtoModalAtual.nome} - ${variacaoModalAtual.nome}`
                    : produtoModalAtual.nome;

                const variacao = variacaoModalAtual?.nome || null;

                // 🔥 VERIFICA SE ESTA VARIAÇÃO ESPECÍFICA JÁ ESTÁ NA LISTA
                if (ListaPedidos.existeVariacao(produtoModalAtual.id, variacao)) {
                    ListaPedidos.remover(produtoModalAtual.id, variacao);
                } else {
                ...
                ListaPedidos.animarBotao(produtoModalAtual.id);
                atualizarBotaoListaModal();
            });
        }
    }

    // Mantém o botão do modal sincronizado caso a lista mude por outro lugar
    document.addEventListener("listaPedidosAtualizada", atualizarBotaoListaModal);
});
/* ==========================================================
   LIGHTBOX
========================================================== */
    ...
    if (indicadores) {
        ...
        if (galeria.length > 1) {
            ...
            galeria.forEach((item, indice) => {
                ...
                miniatura.src = obterUrlMiniatura(item.src, 120);
...
document.addEventListener("click", (e) => {
    ...
    const galeria = card
        ...
        : [{ src: imagem.dataset.full || imagem.src, posicao: imagem.dataset.posicao || "50% 50%" }];

    const indiceClicado = galeria.findIndex(item => item.src === (imagem.dataset.full || imagem.src));
...
/* ==========================================================
   BOTÃO DA LISTA DE PEDIDOS (CORAÇÃO NOS CARDS)
========================================================== */

document.addEventListener("click", async (e) => {

    // 🔥 SÓ PROCESSO CLIQUE EM .botao-lista
    const botao = e.target.closest(".botao-lista");
    if (!botao) return;

    // 🔥 IGNORA SE O BOTÃO ESTIVER DENTRO DO MODAL OU FOR O BOTÃO DO MODAL
    if (botao.closest("#modalVariacoes") || botao.id === "botaoListaVariacao") {
        return;
    }
    ...
    if (ListaPedidos.existe(produtoId)) {
        const produto = await ListaPedidos.buscarProduto(produtoId);
        if (!produto) return;
        ListaPedidos.remover(produtoId);
        ListaPedidos.animarBotao(produtoId);
        return;
    }
    ...
    if (produto.variacoes && produto.variacoes.length > 0) {
        await abrirModalVariacoes(produtoId);
        return;
    }

    // Produto sem variações: adiciona direto
    ListaPedidos.adicionar({ ...produto, nomeCompleto: produto.nome });
    ListaPedidos.animarBotao(produtoId);
...
function atualizarBotoesListaPedidos() {
    document.querySelectorAll(".botao-lista").forEach(botao => {
        // Garante que o texto existe
        let texto = botao.querySelector(".botao-lista-texto");
        if (!texto) {
            texto = document.createElement('span');
            texto.className = 'botao-lista-texto';
        ...
        if (estaNaLista) {
            ...
            texto.textContent = "Adicionar à Lista";
        }

        // 🔥 CORREÇÃO: No desktop, o texto SÓ deve ser visível no hover
        // Não forçamos a visibilidade aqui - deixamos o CSS controlar
        // via :hover ou classes de expansão (mobile)

        // 🔥 Garantir que o texto está com os estilos padrão (invisível)
        // Apenas para desktop - mobile usa classes expandido-*
        if (window.innerWidth > 768) {
            // Desktop: texto invisível por padrão, visível no hover (CSS)
            texto.style.opacity = '';
            texto.style.transform = '';
            texto.style.maxWidth = '';
        } else {
            // Mobile: usa as classes de expansão
            if (botao.classList.contains('expandido-esquerda') || botao.classList.contains('expandido-direita')) {
                texto.style.opacity = '1';
                texto.style.transform = 'scale(1)';
                texto.style.maxWidth = '120px';
            } else {
                texto.style.opacity = '0';
                texto.style.transform = 'scale(0.8)';
                texto.style.maxWidth = '0';
            }
        }
    });
}
...
/* ==========================================================
   ANIMAÇÃO INTELIGENTE DO BOTÃO CORAÇÃO
   Baseado no conceito do TikTok - Leve e eficiente
========================================================== */

// Cache para evitar recriação desnecessária
...
function initHeartAnimation() {
    ...
    setTimeout(configurarAnimacao, 300);
}
...
function configurarAnimacao() {
    ...
    if (observadorLinhas) {
        ...
        observadorLinhas = null;
    }

    // ==== 1. ADICIONA O TEXTO EM CADA BOTÃO ====
    document.querySelectorAll('.botao-lista').forEach(botao => {
        let texto = botao.querySelector('.botao-lista-texto');
        if (!texto) {
            texto = document.createElement('span');
        ...
        const estaNaLista = botao.classList.contains('ativo');
        texto.textContent = estaNaLista ? 'Remover' : 'Adicionar à Lista';
    ...
    // ==== 2. AGRUPA CARDS EM LINHAS (2 por linha) ====
    const linhas = [];
    for (let i = 0; i < cards.length; i += 2) {
        const linha = {
            esquerda: cards[i],
            direita: cards[i + 1] || null,
            ativo: null,
            indice: Math.floor(i / 2)
        };
        linhas.push(linha);
    }

    // ==== 3. FUNÇÃO PARA LIMPAR TODOS OS BOTÕES ====
    function limparTodosBotoes() {
        document.querySelectorAll('.botao-lista').forEach(botao => {
            botao.classList.remove('expandido-esquerda', 'expandido-direita');
            const texto = botao.querySelector('.botao-lista-texto');
            if (texto) {
                texto.style.opacity = '0';
                texto.style.transform = 'scale(0.8)';
                texto.style.maxWidth = '0';
    ...
    // ==== 4. FUNÇÃO PARA ATIVAR UM BOTÃO ====
    function ativarBotao(linha, lado) {
        if (linhaAtiva === linha && linha.ativo === lado) return;

        // Desativa o botão anterior
        if (botaoAtivo) {
            botaoAtivo.classList.remove('expandido-esquerda', 'expandido-direita');
            const textoAntigo = botaoAtivo.querySelector('.botao-lista-texto');
            if (textoAntigo) {
                textoAntigo.style.opacity = '0';
                textoAntigo.style.transform = 'scale(0.8)';
                textoAntigo.style.maxWidth = '0';
            }
        }

        // Ativa o novo botão
        const botao = lado === 'esquerda'
            ? linha.esquerda.querySelector('.botao-lista')
            : linha.direita?.querySelector('.botao-lista');

        if (botao) {
            botao.classList.add(lado === 'esquerda' ? 'expandido-esquerda' : 'expandido-direita');
    ...
    // ==== 5. FUNÇÃO PARA VERIFICAR SE LINHA ESTÁ VISÍVEL ====
    function isLinhaVisivel(linha) {
        const card = linha.esquerda;
        if (!card) return false;

        const rect = card.getBoundingClientRect();
        const windowHeight = window.innerHeight;

        // 🔥 VERIFICA SE PELO MENOS 30% DO CARD ESTÁ VISÍVEL
        const alturaVisivel = Math.min(rect.bottom, windowHeight) - Math.max(rect.top, 0);
        const porcentagemVisivel = alturaVisivel / rect.height;

        return porcentagemVisivel > 0.3;
    }

    // ==== 6. FUNÇÃO PARA ENCONTRAR A LINHA PRINCIPAL ====
    function encontrarLinhaPrincipal() {
        ...
        for (const linha of linhas) {
            ...
            // 🔥 SÓ CONSIDERA LINHAS QUE ESTÃO PELO MENOS 30% VISÍVEIS
            if (!isLinhaVisivel(linha)) continue;

            // 🔥 CALCULA DISTÂNCIA DO CENTRO
            const distancia = Math.abs(centroCard - centroTela);

            // 🔥 SE A LINHA ATUAL ESTIVER VISÍVEL E PRÓXIMA, DÁ PREFERÊNCIA
            if (linhaAtiva && linha === linhaAtiva) {
                // Dá preferência para a linha atual se ela ainda estiver visível
                if (isLinhaVisivel(linha)) {
                    return linha;
                }
            }
    ...
    // ==== 7. INTERSECTION OBSERVER - COM DEBOUNCE E LOGICA MELHORADA ====
    let ultimoTempoTroca = 0;
    const TEMPO_MINIMO_TROCA = 400; // 🔥 400ms entre trocas

    observadorLinhas = new IntersectionObserver((entradas) => {

        // 🔥 DEBOUNCE
        const agora = Date.now();
        if (agora - ultimoTempoTroca < TEMPO_MINIMO_TROCA) {
            return;
        }

        // 🔥 ENCONTRA A LINHA PRINCIPAL
        const linhaPrincipal = encontrarLinhaPrincipal();

        if (!linhaPrincipal) {
            // Se nenhuma linha está visível, limpa tudo
            limparTodosBotoes();
            ultimoTempoTroca = agora;
            return;
        }

        // 🔥 DETERMINA QUAL LADO ATIVAR
        const card = linhaPrincipal.esquerda;
        const rect = card.getBoundingClientRect();
        const centroTela = window.innerHeight / 2;
        const centroCard = rect.top + rect.height / 2;
        const estaAbaixoDoCentro = centroCard > centroTela;

        let ladoParaAtivar = estaAbaixoDoCentro ? 'esquerda' : (linhaPrincipal.direita ? 'direita' : 'esquerda');

        // 🔥 VERIFICA SE JÁ ESTÁ ATIVO
        if (linhaAtiva === linhaPrincipal && linhaPrincipal.ativo === ladoParaAtivar) {
            return; // Já está ativo, não faz nada
        }

        // 🔥 REGISTRA O TEMPO E ATIVA
        ultimoTempoTroca = agora;
        ativarBotao(linhaPrincipal, ladoParaAtivar);

    }, {
        root: null,
        rootMargin: '0px',
        threshold: [0, 0.3, 0.6, 0.9] // 🔥 THRESHOLDS PARA PRECISÃO
    });

    // Observa o primeiro card de cada linha
    linhas.forEach(linha => {
        if (linha.esquerda) {
            observadorLinhas.observe(linha.esquerda);
        }
    });

    // ==== 8. LISTENER ÚNICO PARA ATUALIZAÇÃO DA LISTA ====
    if (!window._listenerListaAnimacaoConfigurado) {
        window._listenerListaAnimacaoConfigurado = true;

        document.addEventListener('listaPedidosAtualizada', () => {
            setTimeout(() => {
                document.querySelectorAll('.botao-lista').forEach(botao => {
                    const texto = botao.querySelector('.botao-lista-texto');
                    if (texto) {
                ...
                if (window.innerWidth <= 768) {
                    setTimeout(() => {
                        if (observadorLinhas) {
                            const entries = observadorLinhas.takeRecords();
                            if (entries.length > 0) {
                                const callback = observadorLinhas.callback;
                                if (callback) {
                                    callback(entries, observadorLinhas);
                                }
                            }
                        }
                    }, 50);
    ...
    console.log('✅ Animação do coração configurada (modo TikTok com linha principal)');
}
...
// Reconecta quando novos cards forem adicionados
// 🔥 Com debounce: evita rodar initHeartAnimation() várias vezes seguidas
// quando várias mutações do DOM acontecem em sequência (ex.: toast, slider)
let _debounceMutacao = null;

const observerMutacao = new MutationObserver(() => {
    if (window.innerWidth <= 768) {
        clearTimeout(_debounceMutacao);
        _debounceMutacao = setTimeout(initHeartAnimation, 300);
    }
});

observerMutacao.observe(document.body, {
    childList: true,
    subtree: true
});